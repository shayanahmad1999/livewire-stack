<div>
    ...
    <form wire:submit="update" class="mx-auto max-w-md flex flex-col gap-6 bg-slate-900 shadow-2xl rounded-2xl p-4">
        ...
        <flux:input wire:model="form.title" :label="__('Title')" type="text" required autofocus autocomplete="title"
            placeholder="Title..." />
        <!-- Image -->
        <flux:input wire:model="form.image" :label="__('Image')" type="file" autofocus />
        <div class="flex gap-2">
            <img src="{{ asset($form->post->image) }}" alt="image" @class(['w-12 h-12 rounded-2xl', 'opacity-30' => $form->image])>
            @if ($form->image)
